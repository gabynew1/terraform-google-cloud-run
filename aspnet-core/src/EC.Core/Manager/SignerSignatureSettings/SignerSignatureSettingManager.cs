...
namespace EC.Manager.SignerSignatureSettings
{
    public class SignerSignatureSettingManager : BaseManager, ISignerSignatureSettingManager
    {
        ...
        public SignerSignatureSettingManager(IWorkScope workScope, ContractManager contractManager, FileStoringManager fileStoringManager) : base(workScope)
        {
        ...
        public async Task<GetContractSignerSignatureSettingDto> GetSignatureSetting(long contractSettingId, string email)
        {
            ...
            var AWSSignedBase64 = await _fileStoringManager.DownloadLatestContractBase64(contract.contractId);
            ...
            return new GetContractSignerSignatureSettingDto
            {
                ...
                ContractBase64 = string.IsNullOrEmpty(currentContractResult) ? (string.IsNullOrEmpty(contract.contractBase64) ?
                AWSSignedBase64 : contract.contractBase64) : currentContractResult,
        ...
        public async Task<GetContractSignerSignatureSettingDto> GetSignatureSettingForContractDesign(long contractId)
        {
            ...
            return new GetContractSignerSignatureSettingDto
            {
                ...
                ContractBase64 = string.IsNullOrEmpty(contract.FileBase64) ? AWSBase64 : contract.FileBase64,
                SignatureSettings = signatureSettings,
            };
        }
        ...
        {
            var result= WorkScope.GetAll<SignerSignatureSetting>()
                .Include(x => x.ContractSetting)
                .ThenInclude(x => x.Contract)
                .Where(x => x.ContractSetting.SignerMassGuid == massGuid && x.ContractSetting.IsSendMail
                 && x.ContractSetting.Contract.Status == ContractStatus.Inprogress).ToList()
                .GroupBy(x => x.ContractSettingId)
                .Select(x => new GetMassContractNotSignDto
                {
                    ContractSettingId = x.Key,
                    ContractId = x.Select(x => x.ContractSetting.ContractId).FirstOrDefault(),
                    Signature = x.Select(y => new GetMassContractNotSignSignatureDto { SignerSignatureSettingid = y.Id, SignatureType = y.SignatureType }).ToList(),
                    IsSigned = x.All(y => y.IsSigned)
                }).ToList();
